...
data_path = os.path.join(ROOT_FOLDER, "data", "teste_externo", "dataset_interno_teste_externo.xlsx")
...
def change_dataset(data_path):
    ...
    output_dir = os.path.join(ROOT_FOLDER, "data", "teste_externo", "datasets_tratados")
    os.makedirs(output_dir, exist_ok=True)
    
    # define output paths
    base_name = "dataset_teste_externo"
    ...
    if not os.path.exists(df_by_contract_path):
        ...
        rename_map = {
            ...
            'Cargo': 'Cargo',
            'Descrição da Profissão': 'descricao_da_Profissao',
            'Cidade (Loja)': 'Cidade_Loja',
            'Modalidade da Proposta': 'Modalidade_da_Proposta',
            'Valor Inicial da Prestação': 'valor_inicial_da_prestacao',
            'Valor Financiado': 'valor_financiado',
            'Renda do Cliente': 'renda_cliente',
            'Plano de Financiamento': 'plano_financiamento',
            'Data Venc. 1ª Parcela': 'Data_inicio'
        }
        df = df.rename(columns=rename_map)
        
        # Ensure Data_inicio is datetime
        df['Data_inicio'] = pd.to_datetime(df['Data_inicio'])
        
        # Calculate derived features
        df['salario_perc'] = df.apply(lambda row: row['valor_inicial_da_prestacao'] / row['renda_cliente'] if row['renda_cliente'] != 0 else 1, axis=1)
        
        # Clean City names
        def clean_city(city):
            if pd.isna(city): return ""
            city = str(city).title().rstrip()
        ...
        df['Cidade_Loja'] = df['Cidade_Loja'].apply(clean_city)
        
        # save new dataset
        df.to_excel(df_by_contract_path, index=False)

    if not os.path.exists(df_classified_products_path):
        df = pd.read_excel(df_by_contract_path)

        # analisys(df) # Skipped because we don't have profit info
    ...
    if not os.path.exists(df_augmented_path):
        ...
        # Rename Data_inicio to Data_Lancamento for add_ipca compatibility if needed, 
        # but add_ipca uses Data_Lancamento. Let's check add_ipca.
        # add_ipca uses df["Data_Lancamento"].
        df["Data_Lancamento"] = df["Data_inicio"]

        df = add_ipca(df, df_ipca)
        df = add_idh_info(df, df_idh)
        
        # Drop Data_Lancamento if it was just a duplicate
        # df = df.drop(columns=["Data_Lancamento"]) 
    ...
    if not os.path.exists(df_cb_longbasica_path):
        ...
        # 2022 file has a different header structure
        df_cb_22  =  pd.read_excel(os.path.join(ROOT_FOLDER, 'data', 'external_data', 'cesta_basica_capitais', 'raw_data', 'cesta_basica_dieese_2022_012022_122022.xls'), header=1)
        df_cb_22 = df_cb_22.rename(columns={'Unnamed: 0': 'data'})
        ...
        def process_cb_year(df_cb, df, year):
            ...
            df_cb_long["data"] = pd.to_datetime(df_cb_long["data"], format="%m-%Y", errors='coerce')
            df_cb_long = df_cb_long.dropna(subset=["data"])
            ...
            # Apenas linhas do ano especifico
            ...
            for uf in df_year["uf"].unique():
                ...
                if right.empty:
                    print(f"Estado {uf} sem dados de cesta básica no ano {year}. Mantendo linhas com NaN.")
        ...
        df_2022 = process_cb_year(df_cb_22, df, 2022)
        ...
        # Rename valor_cestabasica to preco_cesta_basica for consistency
        df_final = df_final.rename(columns={'valor_cestabasica': 'preco_cesta_basica'})

        df_final['preco_cb_perc'] = df_final['preco_cesta_basica']/df_final['renda_cliente']
    ...
    # Removed 'lucro' from critical columns
    critical_columns = ['valor_inicial_da_prestacao', 'valor_financiado', 
